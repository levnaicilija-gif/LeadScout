...
**Rules:**
- **Grep for `NEW.<column>` and `OLD.<column>` before dropping a column.** A trigger function that reads a dropped field appears in NO catalogue dependency the drop consults: the database will not refuse it and will not warn, because plpgsql resolves the field only when the trigger RUNS. The failure arrives on the next INSERT, as a bare `42703 record "new" has no field "…"`, and it takes every write to that table with it. Dependency checks on indexes, constraints, policies and column-reading functions all pass cleanly while this waits.
...
- A column dropped without grepping for `NEW.<column>` / `OLD.<column>` in trigger functions — no dependency check sees those, and the drop succeeds silently while the next INSERT fails
