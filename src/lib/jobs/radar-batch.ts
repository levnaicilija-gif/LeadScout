...
/**
 * One batch of Radar, run by /api/jobs/tick (src/lib/jobs/tick.ts) from 04:00 UTC. Reads sources, fetches unseen links, extracts leads under Stage 1 rules,
...
export async function runRadarBatch(req: Request) {
  ...
  // Chunking: a Vercel function has 300 s, which is not enough for many sources. Each call takes
  // `batch` sources from `cursor`; the tick calls again from where the last batch stopped.
  ...
  // One batch, and nothing handed on. Batches used to pass the next one on over HTTP, and on 14 September
  // 2026 every batch of the morning run started within two seconds of the first; Vercel refused the fifth
  // with 508 INFINITE_LOOP_DETECTED, because a function calling its own deployment is capped however it is
  // timed. The tick runs batches one after another instead.
  //
  ...
  if (runRow) {
    await db.from('radar_runs').update({
      // more: whether sources remain after this batch. The tick reads it to find where today's run stands.
      finished_at: new Date().toISOString(), tally: { ...tally, more }, rejected, sources_seen: audit,
    }).eq('id', runRow.id);
  }

  // One line per batch, so the daily run is readable in the Vercel logs.
  console.log(`[radar] cursor=${cursor} batch=${take} sources=${tally.sources} articles=${tally.articlesRead} leads=${tally.leads} rejected=${tally.rejected} filtered=${tally.filtered} lowConfidence=${tally.lowConfidence} model=€${tally.modelEur}${tally.budgetStopped ? ` STOPPED at the €${cap} cap (${tally.linksNotRead} links not read)` : ''} next=${more ? cursor + take : 'done'}${tenders ? ` tenders=${tenders.error ? `error: ${tenders.error}` : `${tenders.leadsCreated} leads from ${tenders.noticesChecked} notices`}` : ''}`);

  return NextResponse.json({ ok: true, browser: browserProvider(), cursor, batch: take, nextCursor: more ? cursor + take : null, tally, tenders, audit, rejected, report });
}
