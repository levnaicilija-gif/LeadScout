...
${job ? `A job is attached, and so is the score against it. Write ONLY the questions that come out of this score — the standard set (certificates and expiry, rotation, medical and safety training, English on site, start and notice, rate, conflicts) is added afterwards in code, so do not write those and do not count them against your number.

In this order: every blocker gets a question; then every item in "missing", asking what would close the gap rather than whether it exists — the recruiter can already see that it does; then every item in "fits" that rests on the candidate's own say-so rather than on a confirmed document, asking for the specifics that would prove it — the standard, the thickness, the process, the structure, the acceptance criteria. A fit nobody has probed is what falls apart in front of the client.` : 'No job is attached, so cover the trade: processes, positions and standards; certificates and expiry; rotation history; offshore medical and safety training; passport, A1 and right to work; English on site; rate and earliest mobilisation.'}
