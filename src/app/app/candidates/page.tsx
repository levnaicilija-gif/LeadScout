...
export default async function Candidates({ searchParams }: { searchParams: { q?: string; ref?: string } }) {
  const sb = supabaseServer(); let q = sb.from('candidates').select('*, documents(type, status, cert_body, verifications(result, valid_until, checked_where, checked_at)), sends(sent_at, revealed_at, companies(name))').order('created_at', { ascending: false }).limit(100);
  ...
  return (<>
    ...
    <div className="bg-panel border border-line rounded overflow-auto"><table className="tbl w-full min-w-[980px] border-collapse"><thead><tr><th>Reference</th><th>Trade</th><th>Certificates</th><th>Docs</th><th>Availability</th><th>Sent to</th><th>Languages</th><th>Origin</th></tr></thead><tbody>
      {(data ?? []).map((c: any) => { const certs = (c.documents ?? []).filter((d: any) => d.type === 'certificate'); return <tr key={c.id}>
        ...
        <td>{(c.documents ?? []).length}</td><td>{c.availability_from ?? '—'}</td><td>{(c.sends ?? []).map((s: any) => s.companies?.name).join(', ') || '—'}</td><td>{(c.languages ?? []).join(', ')}</td><td className="text-ink3 text-[12px]">{c.created_via} · {new Date(c.created_at).toLocaleDateString()}</td></tr>; })}
      {(data ?? []).length === 0 && <tr><td colSpan={8} className="p-6 text-ink3">No candidates yet. Drop a CV into Verify → CV anonymizer.</td></tr>}
    </tbody></table></div>
  </>);
}
