...
export default async function Pitch() {
  ...
  // then are flattened so PitchClient keeps reading `lead.confirmed_at` unchanged. confirmed_at is
  // NOT named on the parent: the embed is `!inner`, so every row has a state row to take it from,
  // and naming the old column too would hide a reference 2c removes inside a template literal, which
  // column-exists-check cannot read.
  const [{ data: cands }, { data: leadRows }] = await Promise.all([sb.from('candidates').select('id, reference_code, trade, profile, availability_from').limit(200), sb.from('leads').select(`id, project_name, trades_inferred, project_location, companies(name, employer_type), contacts(name, title, email_status), ${LEAD_STATE_EMBED}`).not(`${LEAD_STATE_TABLE}.status`, 'in', CLOSED_LEAD_STATUSES).limit(200)]);
