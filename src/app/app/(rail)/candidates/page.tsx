...
export default async function Candidates({ searchParams }: { searchParams: { q?: string; ref?: string } }) {
  ...
  const sb = supabaseServer(); let q = sb.from('candidates').select('*, documents!candidate_id(type, status, cert_body, verifications(result, valid_until, checked_where, checked_at)), sends(sent_at, revealed_at, companies(name)), anonymized_cvs(storage_path, pii_check_passed)').order('created_at', { ascending: false }).limit(100);
  ...
  return (<>
    <div className="flex items-baseline justify-between flex-wrap gap-x-3 gap-y-1 mb-3"><h1 className="font-display text-[26px] font-bold tracking-[-.4px]">Candidates<Help title="What Candidates is" intro="The pool. Full name, phone and documents stay here, internal only." rows={[['Where they come from', 'Only from you: Verify → Save, the Add button, or imported records. Radar never creates candidates.'], ['Reveal', 'Releasing a full profile to a client is an explicit, logged action.']]} /></h1><span className="text-ink3">{error ? 'could not read the pool' : `${data?.length ?? 0} shown`}</span></div>
    {error && <div className="bg-panel border border-bad rounded-card p-4 mb-3 text-[13px]">
      ...
      <details className="mt-1 text-[12px] text-ink3"><summary className="cursor-pointer">Technical detail</summary><pre className="whitespace-pre-wrap mt-1">{error.message}</pre></details>
    </div>}
    ...
    <div className="bg-panel border border-line rounded-card overflow-auto"><table className="tbl w-full min-w-[980px] border-collapse"><thead><tr><th>Reference</th><th>Trade</th><th>Certificates</th><th>Docs</th><th>Availability</th><th>Sent to</th><th>Languages</th><th>Origin</th><th>Client CV</th></tr></thead><tbody>
      ...
      {!error && (data ?? []).length === 0 && <tr><td colSpan={9} className="p-6 text-ink3">No candidates yet. Drop a CV into Verify → CV anonymizer.</td></tr>}
    </tbody></table></div>
  </>);
}
