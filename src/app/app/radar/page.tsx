...
export default async function Radar({ searchParams }: { searchParams: { tab?: string; lead?: string } }) {
  ...
  return (<>
    ...
    <div className="bg-panel border border-line rounded overflow-auto max-h-[calc(100vh-190px)]"><table className="tbl w-full min-w-[1100px] border-collapse">
      ...
      {(leads ?? []).length === 0 && <tr><td colSpan={7} className="text-ink3 p-6">{tab === 'job_post'
        ? 'No company postings yet — Hiring now runs daily once careers pages are discovered.'
        : 'No leads yet. Radar reads your sources every morning at 06:00.'}</td></tr>}
      </tbody></table></div>
