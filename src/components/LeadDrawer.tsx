...
export function LeadDrawer({ lead }: { lead: any }) {
  ...
  return (<aside className="fixed top-0 right-0 h-screen w-[500px] bg-panel border-l border-line shadow-[-16px_0_48px_rgba(14,26,43,.12)] overflow-auto p-6 pb-12 z-10">
    ...
    <div className="mt-3 text-[13px]">
      {tool === 'jd' && <><button className="btn btn-primary" disabled={!!busy} onClick={async () => setOut({ ...out, jd: await call('jd') })}>{busy === 'jd' ? 'Writing…' : lead.job_description ? 'Rewrite JD' : 'Write JD'}</button><Failure action="jd" />{(out.jd?.job_description ?? lead.job_description) && <div className="mt-2 border border-line rounded bg-[#FAFBFC] p-3 whitespace-pre-wrap">{out.jd?.job_description ?? lead.job_description}{out.jd?.assumptions?.length > 0 && <div className="mt-2 text-warn text-[12px]">Assumed (confirm on the call): {out.jd.assumptions.join(' · ')}</div>}</div>}</>}
      ...
      {tool === 'xray' && <><button className="btn btn-primary" disabled={!!busy} onClick={async () => { const j = await call('xray'); if (j.url) window.open(j.url, '_blank'); }}>{busy === 'xray' ? 'Building…' : 'Open LinkedIn search'}</button><Failure action="xray" /><div className="text-ink3 text-[12px] mt-2">Built from the JD, English only. Drop chosen CVs into Verify → they score against this lead.</div></>}
      {tool === 'q' && <><button className="btn btn-primary" disabled={!!busy} onClick={async () => setOut({ ...out, q: await call('questions') })}>{busy === 'questions' ? 'Writing…' : 'Get questions'}</button><Failure action="questions" />{out.q?.questions?.length === 0 && <div className="text-ink3 mt-2">No questions came back. Try again.</div>}{out.q?.questions?.length > 0 && <ol className="list-decimal pl-5 mt-2">{out.q.questions.map((q: any, i: number) => <li key={i} className="mb-2"><b className="font-medium block">{q.q}</b><small className="text-ink3">Good: {q.good_answer}</small></li>)}</ol>}</>}
    </div>
