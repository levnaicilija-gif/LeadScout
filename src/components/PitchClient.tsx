...
export function PitchClient({ candidates, leads }: { candidates: any[]; leads: any[] }) {
  ...
  return (<div className="grid grid-cols-[minmax(340px,1fr)_minmax(0,1.4fr)] gap-4 items-start">
    <div className="bg-panel border border-line rounded"><div className="px-4 py-3 border-b border-line font-semibold">Candidate</div><div className="p-4 text-[13px]">
      ...
      {c && <div className="grid grid-cols-[110px_1fr] gap-y-1.5 mt-3"><span className="text-ink3">Projects</span><span>{(c.profile?.projects ?? []).map((p: any) => `${p.type}, ${p.country} ${p.years}`).join('; ') || '—'}</span><span className="text-ink3">Languages</span><span>{(c.profile?.languages ?? []).join(', ')}</span></div>}
      <button className="btn btn-primary mt-4" onClick={match} disabled={!c}>Match</button>
    </div></div>
    <div className="bg-panel border border-line rounded"><div className="px-4 py-3 border-b border-line flex justify-between"><b className="font-semibold">Who should hear about this person</b><span className="text-ink3 text-[12px]">trade · prior site · client type</span></div>
